# Multi-Model Adjudication and Runtime-Provenance Research Status

> Non-execution-source live wayfinding. `current/human-approved-spec.md` remains Mnemosyne's only execution source.

```yaml
status_id: MULTI-MODEL-ADJUDICATION-PROVENANCE-STATUS-003
last_status_task: MNEMOSYNE-147
cycle: raw/research-reports/cycles/2026Q3-multi-model-adjudication-provenance
trusted_progress_checkpoint:
  record: current/multi-model-adjudication-provenance-reliable-progress-checkpoint.md
  effective_after_merge_commit: 94de7427da56659f472cbc11eb1bf310d5b6116a
canonical_DR07_pair:
  primary_research_candidate: independent_labeled_Pro
  control: independent_labeled_Thinking_RESEARCH_INCOMPLETE_REPOSITORY_ACCESS
  project_internal_pair_used: false
heterogeneous_follow_up:
  study_id: FABLE5-GOV-001
  path: raw/research-reports/cycles/2026Q3-multi-model-adjudication-provenance/fable5-governance-001
  storage_PR: 197
  storage_status: merged
  storage_merge_commit: 7434407f34caa5aa576c36bdf70adec84b2205d2
  role: heterogeneous_corroboration_and_recovery_design_enhancement
  prior_DR07_full_reports_read: false
  complete_blinding: false_due_to_disclosed_incidental_exposure
  portable_source_manifest_complete: false
recommendation_adoption:
  task: MNEMOSYNE-147
  canonical_PR: 198
  repository_status: pending_human_merge_of_PR_198
  adopted_scope:
    - compact_run_record_for_important_repository_writes
    - concise_natural_language_disclosure_for_low_risk_work
    - PR_execution_context_disclosure
    - operator_selection_provider_mapping_backend_identity_separation
    - model_or_surface_switch_recording
    - reviewer_independence_recording
    - high_impact_execution_source_or_trust_boundary_heterogeneous_review_unless_user_exception
    - later_reliable_Pro_or_stronger_model_review_marker
  active_guard: current/run-context-and-pr-provenance-guard.md
  guidance_loader_updated: true
  maturity: provisional_pending_reliable_Pro_or_stronger_model_review
  current_task_official_context:
    product_surface: standard_ChatGPT_conversation
    operator_selected_option: Extra_High
    provider_documented_model_mapping: GPT_5_6_Sol
    backend_model_identity: UNKNOWN_OR_NOT_ATTESTABLE
  deferred:
    - full_T0_T5_global_framework
    - complete_checkpoint_field_expansion
    - automatic_checkpoint_activation
    - heavyweight_cryptographic_provenance
    - GF_STEP_5_substantive_adjudication
actual_backend_model_identity_proven: false
execution_source_modified_by_MNEMOSYNE_147: false
GF_STEP_5_substantive_adjudication: not_started
next_gate:
  - human_review_and_merge_PR_198
  - use_the_new_guard_for_subsequent_important_PRs_after_merge
  - later_review_and_improve_the_guard_with_a_reliably_available_Pro_or_stronger_model
  - separately_decide_source_portability_and_GF_STEP_5_adjudication
  - do_not_treat_UI_or_provider_mapping_as_particular_run_backend_attestation
```

## Current interpretation

The research-to-policy route has now moved from evidence receipt to a focused first adoption: a compact, risk-based run-context and pull-request provenance behavior guard. This adoption is narrower than the complete FABLE5-GOV-001 proposal and is marked provisional for later stronger-model review.

The current official standard-ChatGPT terminology used for MNEMOSYNE-147 is `GPT-5.6 Sol / Extra High`; the particular response's backend identity remains `UNKNOWN_OR_NOT_ATTESTABLE`.

Source portability, complete checkpoint-schema expansion, and substantive Fable Greenfield adjudication remain separate future gates.
