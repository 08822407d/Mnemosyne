...
18. 是否需要把 7 份研究报告的研究动机入库？
   - 结论：需要。MNEMOSYNE-030C 创建 `raw/research-reports/cycles/2026Q2-initial/research-cycle-origin-and-motivation.md`。
   - 状态：answered；仍需用户 review motivation 文件。

19. 研究课题 prompt 原文应该放在哪里？
   - 结论：pro 深度研究 prompt 原文约定放在 `raw/research-reports/cycles/2026Q2-initial/research-prompts/originals/AI_agent_external_persistent_memory_deep_research_prompt_pro.md`，且当前文件存在。
   - 状态：answered；仍需用户 review prompt mapping。

20. 6 个轻度研究 prompt 原文缺失时应如何处理？
   - 结论：不得编造原文；只记录 report title / summary 可确认的 topic title，并标记 `missing_original_prompt`。
   - 状态：answered；MNEMOSYNE-030D / 030F 已在 research prompt index / report-topic map 中显式记录缺失状态。


21. 是否需要同步 research motivation / prompt mapping 到 current 索引与接手文件？
   - 结论：需要。MNEMOSYNE-030F 用于补齐 current 索引、active-context、handoff、todo、open-questions 的状态。
   - 状态：answered；后续仍需用户 review motivation / prompts / summaries。

## open

0a. 用户是否接受 `raw/research-reports/current/current-research-prompts.md` 与 `raw/research-reports/cycles/2026Q2-initial/report-topic-and-prompt-map.md`？
   - 说明：MNEMOSYNE-030D 已建立 research prompts / topic mapping 层，仍需用户 review。

0b. 如果未来找回轻度研究 prompt，是否补入 `research-prompts/originals/` 并更新索引？
   - 说明：当前 6 个轻度研究 prompt 均标记为 `missing_original_prompt`，不得编造原文。

0c. 是否需要把 prompt 原文与 report summary 的差异做 delta / review note？
   - 说明：prompt 是研究输入，report / summary 是研究结果或派生证据；如果二者差异影响理解，可登记 delta / review note。

0d. 用户是否接受当前 research motivation 文件？
