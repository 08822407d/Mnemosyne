...
- 定位：这些文件属于 prompt / topic mapping 层，不是执行源，也不是研究报告结果；prompt 是研究输入，report / summary / evidence map 是研究结果或派生证据。
- 缺失 prompt 处理：6 个轻度研究 prompt 原文缺失，只记录 topic title / inferred topic，不伪造原文。
