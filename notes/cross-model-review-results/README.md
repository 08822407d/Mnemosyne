...
- `FABLE5-GREENFIELD-001/` — Fable 5 independent greenfield reconstruction track. The stored charter defines a source firewall, atomic multi-step plan, raw-preservation requirements, and a later comparison phase. It is a contrastive reference track, not a replacement for the current GPT design or execution source.
  ...
  - `steps/GF-STEP-3A/` — bounded information-architecture and authority-model design result. The exact task and exact returned Markdown are archived with byte-level reconstruction metadata, together with the Fable summary and step manifest. Fable reports 18 architecture elements, six alternative sets, 15 explicit design parameters, 21-need coverage, 24 verified-boundary checks, and 16 unsupported-assumption guards. The result remains advisory and unaccepted; GF-STEP-3B is proposed but not executed.
  - `steps/GF-STEP-3-EARLY/` — byte-faithful premature architecture candidate returned during the initial misinterpretation of GF-STEP-2D. It remains preserved but unaccepted as the canonical GF-STEP-3 result and was not used by the canonical GF-STEP-3A execution.
