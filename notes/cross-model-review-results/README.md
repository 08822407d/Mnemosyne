...
- `FABLE5-GREENFIELD-001/` — Fable 5 independent greenfield reconstruction track. The stored charter defines a source firewall, atomic multi-step plan, raw-preservation requirements, and a later comparison phase. It is a contrastive reference track, not a replacement for the current GPT design or execution source.
  ...
  - `steps/GF-STEP-3R/` — bounded repair task plus the first failed execution attempt are stored. The exact task is recoverable from a deterministic archive; Fable stopped with `GF_STEP_3R_incomplete_input_integrity_failure` because the required STEP4 self-critique attachment was absent. No design file was read, no amendment or repair was produced, and a clean rerun of the same task with exactly five required design attachments remains the sole continuation.
