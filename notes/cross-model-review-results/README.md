...
- `FABLE5-GREENFIELD-001/` — Fable 5 independent greenfield reconstruction track. The stored charter defines a source firewall, atomic multi-step plan, raw-preservation requirements, and a later comparison phase. It is a contrastive reference track, not a replacement for the current GPT design or execution source.
  ...
  - `steps/GF-STEP-2D/` — exact corrective task, the initial misinterpreted attempt, the successful fresh-conversation rerun summary, and the exact rerun output archive are stored. Fable reports eight verified canonical reads and `GF_STEP_2_complete_with_dated_fact_and_text_only_visual_caveats`; this remains advisory pending substantive maintainer acceptance. The bounded next proposal is GF-STEP-3A, not executed.
  - `steps/GF-STEP-3-EARLY/` — byte-faithful premature architecture candidate returned during the initial misinterpretation of GF-STEP-2D. It remains preserved but unaccepted as the canonical GF-STEP-3 result.
