...
- `FABLE5-GREENFIELD-001/` — Fable 5 independent greenfield reconstruction track. The stored charter defines a source firewall, atomic multi-step plan, raw-preservation requirements, and a later comparison phase. It is a contrastive reference track, not a replacement for the current GPT design or execution source.
  ...
  - `steps/GF-STEP-2B4B/` — completion of the hosted-repository workflow PDF text-layer review. The exact prompt is preserved in three ordered multipart files with a concatenation index, alongside the chat summary, byte-faithful downloadable output, integrity metadata, five final evidence records replacing the B4A provisional records, the final S-05 refinement, repository-evidence and placement maps, and the STEP2B5 continuation. No OCR or visual inspection was performed; GF-STEP-2 remains incomplete and substantive acceptance remains deferred.
