...
## F. 批次四消化（MNE-DR-028 总体模型复核）

1. **类比未被否定、但五个"假保证"须拆除**：模型确定性、上下文可随机寻址、指令/数据天然隔离、规则文本自动强制、交接包=完整进程快照——五项均有直接反证文献。修正方向："概率计算后端＋确定性治理边界＋显式耐久状态"。
2. **六候选无全胜者**；证据最一致的方向是**分层**（权威历史/当前投影/工作态/上下文装载/概率计算/确定性治理分离）——与设计稿 v1 的五层三面**结构性合流**（我们的 L0+L1 append-only 裁决层≈轻量账本；L2/L4≈投影；检查点≈应用级 checkpoint）。
3. **对设计稿的三处强化输入**：①"规则像程序"只在组织层成立——**必须阻断的规范必须有模型外强制层**（hook/权限门），恰与 027 实测的零审批卡写入风险互证；②working state 与 prompt context 须显式分离（context=从工作态编译的有限注意力工作集）；③投影写向单向化（人不直接改投影、改经裁决），防双真相源。
4. **路线裁定材料**：沿用心智模型＋定点修补=中高支持；分层混合=中高支持但多年实证 UNKNOWN——设计稿 v1 实为两者合流体（保留类比作导览、机制上已分层），v2 予以明文化。
5. UNKNOWN 诚实清单齐全（无六候选 head-to-head；无跨年迁移保真实证）——列为长期观察项，不阻塞设计。
