...
## 9. 第三批（2026-08-31 晚，Owner 指示优先消耗当晚刷新前的 Pro 额度；服务于阶段 1 可行性分析）

| ID | 课题 | 统一号 | 执行位置 | 影响的决定 |
|---|---|---|---|---|
| RQ8（解除 DEFER） | 学习者建模与教学计划生成证据 | **MNE-DR-025** | 深度研究 Pro | N-08（学习类 agent 目标）的可行性三档判定——此判定属阶段 1 当前工作，非目标项目设计；题面已限定为耐久证据问题以避免被未来立项决定作废 |
| RQ9（解除 DEFER） | 开发知识资产与自用代码库实践 | **MNE-DR-026** | 深度研究 Pro | N-07（代码开发 agent 目标）的可行性三档判定；同上 |
| RQ12（新增） | ChatGPT GitHub 写能力实测 | **MNE-DR-027** | **普通对话 Pro（非深度研究）** | 020 报告"官方称只读"与本仓 7 月实测写入的矛盾定分晓；决定阶段 2 中 ChatGPT 侧写入路径设计＋返场预案 P4 |
| RQ10/RQ11 | 次档评估方法 / 数字分身证据 | — | 维持 DEFER | 前者属实验设计非查证；后者待 Owner 定隐私边界 |

编号 025/026/027 按统一序列取号（next→028）；DEFER 解除理由：Owner 指示消耗额度＋两题实为阶段 1 可行性分析的证据输入（原 DEFER 理由针对"设计题"，现题面已收窄为"证据题"）。

## 10. X-1 裁定衍生课题（2026-08-31 深夜）

| ID | 课题 | 统一号 | 状态 | 影响的决定 |
|---|---|---|---|---|
| RQ13 | 总体架构抽象模型复核——"模型算/文件记"类比之外是否存在更适合的总体模型（认知架构、事件溯源、状态机中心、平台原生混合等候选的实证对比） | **待分配**（取号冻结中，#325 合并解除后拟取 028） | 任务书待冻结解除后交付 | Owner X-1 裁定的附带指令：早期无依据构想须回头复核；阶段 2 架构总览的候选面宽度 |
