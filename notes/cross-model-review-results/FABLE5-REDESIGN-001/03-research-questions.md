...
## 8. 第二批课题（2026-08-31，Owner 要求继续消耗 Pro 额度）

| ID | 课题 | 统一号 | 状态 | 影响的决定 |
|---|---|---|---|---|
| RQ3（升格） | 需求生命周期与状态演化管理 | **MNE-DR-022** | READY——taskbook `project-knowledge/FABLE5-REDESIGN-001/MNE-DR-022-requirements-lifecycle-taskbook.md` | 阶段 2 需求生命周期状态机；N-04/N-05/N-14 验收标准。原 DEFER 理由（等登记表确认）经复估解除：外部证据面与逐条确认结果基本独立 |
| RQ6 | 文件/Git 真相源上的检索与按需加载 | **MNE-DR-023** | READY——`MNE-DR-023-retrieval-and-loading-taskbook.md` | 阶段 2 加载/投影机制（记录≠加载的机械化）；021 报告揭示的"检索层缺口"是否补、怎么补 |
| RQ7 | 交接效果评测工具与小样本测试方法 | **MNE-DR-024** | READY——`MNE-DR-024-handoff-eval-instruments-taskbook.md` | 阶段 2 交接方案的预冻结效果测试设计；与 021 报告分工：021 给结论，024 给落地工具与方法 |
| RQ8 | 语言教师/学习者建模证据（原 RQ4 细化） | — | DEFER：目标项目线未立项（#265 Q2 挂起：Meta-Agent 先行） | 未来教师 agent 记忆系统设计 |
| RQ9 | 开发知识资产/自用代码库实践 | — | DEFER：同上（真实需求 A 未立项） | 未来代码库 agent 设计 |
| RQ10 | 次档模型可靠性评估方法 | — | DEFER：#265 TODO 3 未立项；属实验设计而非纯外部证据 | 次档验证实验设计 |
| RQ11 | 数字分身/个人思维建模证据 | — | DEFER：Alaya 线；画像敏感度边界须先由 Owner 定 | Alaya 分析线设计 |

编号分配：022/023/024 已按统一序列取号（next→025）；注册表补记随既有补丁一并交维护线。
