...
| ID | 课题 | 统一号 | 状态 | 影响的决定 |
|---|---|---|---|---|
| RQ3（升格） | 需求生命周期与状态演化管理 | **MNE-DR-022** | 已执行，报告已回收（06-research-received/） | 阶段 2 需求生命周期状态机；N-04/N-05/N-14 验收标准。原 DEFER 理由（等登记表确认）经复估解除：外部证据面与逐条确认结果基本独立 |
| RQ6 | 文件/Git 真相源上的检索与按需加载 | **MNE-DR-023** | 已执行，报告已回收（06-research-received/） | 阶段 2 加载/投影机制（记录≠加载的机械化）；021 报告揭示的"检索层缺口"是否补、怎么补 |
| RQ7 | 交接效果评测工具与小样本测试方法 | **MNE-DR-024** | 已执行，报告已回收（06-research-received/） | 阶段 2 交接方案的预冻结效果测试设计；与 021 报告分工：021 给结论，024 给落地工具与方法 |
