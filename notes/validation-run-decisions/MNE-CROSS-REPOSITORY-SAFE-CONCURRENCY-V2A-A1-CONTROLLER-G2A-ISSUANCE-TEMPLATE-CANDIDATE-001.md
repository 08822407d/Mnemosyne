NON_AUTHORIZING_TEMPLATE — DO NOT SEND THIS WHOLE FILE TO A CONTROLLER.

# V2-A A1 Controller G2A Issuance Template Candidate 001

```yaml
artifact_id: MNE-CROSS-REPOSITORY-SAFE-CONCURRENCY-V2A-A1-CONTROLLER-G2A-ISSUANCE-TEMPLATE-CANDIDATE-001
produced_by: current_ChatGPT_Pro_adjudication
source_fable_candidate:
...
status: Pro_corrected_non_authorizing_outer_template
direct_issue_status: BLOCKED_UNTIL_REPOSITORY_PUBLICATION_READBACK_DYNAMIC_FILL_AND_SEPARATE_OWNER_G2A
G2A_issued: false
...
inner_payload_template:
  ...
  line_endings: LF
  bytes: 25109
  sha256: bda27d39dcfe0159523721e5c8831f86886031cb3d4ba4365979ea3a5245ddd4
```

This outer document is evidence and a transfer artifact only. It cannot authorize G2A or A1. The future Owner must send **only** the text inside the `OWNER_G2A_PAYLOAD_TEMPLATE_BEGIN/END` envelope after:

1. this template is published to Mnemosyne through a reviewed Ready PR;
2. the merged template path/blob is read back;
3. the current dynamic fields are filled from direct Owner/operator evidence;
4. a mechanical fill validator confirms that only the authorized G2A-time placeholders changed;
5. the Owner separately decides to issue G2A.

The envelope markers themselves are not part of the message sent to the controller.

## Pro corrections applied to the Fable candidate

- separated the permanently non-authorizing evidence wrapper from the future authority-bearing payload;
- removed the self-negating `G2A_authorized: false` / “not an Owner G2A” clauses from the issueable payload;
- added a publication-time self-blob field supplied only after this template's merged blob exists;
- normalized worker selected-label state to the Package 002 token `not_yet_observed`;
- dropped the package-unsupported `<G2A_TIMESTAMP_UTC>` field;
- retained the Package 001 V1 ref inventory by exact manifest-blob reference rather than duplicating a second mutable list;
- kept both Package 003 canonical wrapper blocks byte-identical.

## Exact future issuance payload template

OWNER_G2A_PAYLOAD_TEMPLATE_BEGIN
@GitHub

这是 `MNE-CROSS-REPOSITORY-SAFE-CONCURRENCY-V2A-A1-001` 的 Owner G2A 授权，也是 fresh A1 controller 的唯一正式启动消息。

本消息只授权执行 V2-A 的 A1 positive independent pair。只有在本消息内全部动态字段已由 Owner/操作员按当前证据填充、controller 完成完全只读 preflight 且精确 PASS 后，才允许进入冻结的五分支执行流程。任何缺失、不一致、未知或工具能力不足均必须 fail closed；不得刷新预期值、修包、换模型或重试。

## 1. Controlling identities (static; bound by this issued message)
...
## 4. Issuance-time dynamic bindings and placeholder discipline

[C-08] The following values are bound only when the Owner actually sends this G2A message. Every angle-bracket field below must be replaced before sending. The two `__MNE_*_AT_LAUNCH__` worker-selected-label placeholders inside the canonical wrapper templates must remain unresolved until their exact worker launches.

```yaml
Owner_G2A_authorized: true
source_controller_G2A_template:
  ...
  blob: <SOURCE_CONTROLLER_G2A_TEMPLATE_BLOB>
...
controller_model_binding:
  Owner_authorized_visible_label: <CONTROLLER_OWNER_AUTHORIZED_VISIBLE_LABEL>
  operator_selected_visible_label: <CONTROLLER_OPERATOR_SELECTED_VISIBLE_LABEL>
...
The controller must block before branch creation if any issuance-time placeholder remains unresolved, either protected master differs from the value above, the controller label pair is missing or not byte-identical, either worker authorized label is missing, or a worker selected label is asserted as already observed.
...
## 13. Owner G2A authorization closure

This exact message authorizes only the scope and conditional sequence stated above. It does not authorize A2–A7, V2-B, V2-C, any real target, any Mnemosyne or Meta-Agent write during A1, any PR, any retry, any repair, or any cleanup. The controller must stop after the frozen final bundle and return the complete result to a fresh Pro adjudicator.
