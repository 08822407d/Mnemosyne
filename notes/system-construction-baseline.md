...
- Codex Task Result Record 默认占位符路径已由 MNEMOSYNE-025H 规范化为 `notes/codex-task-results/TASK_ID-result.md`；
- `notes/self-improvement-workflow.md` 仍可在后续进行 Markdown 格式清理，但这不阻断路径正确性；
- `notes/overall-target-and-roadmap-snapshot.md` 中缺失 TASK_ID 或尖括号占位符的旧路径表述已由 MNEMOSYNE-025H 修正；
