...
- 内容：需要完成 research motivation / prompt mapping 状态硬同步，确保 current 索引、active-context、handoff、todo、open-questions 和 task result records 都能作为后续接手入口。
- 状态：reflected
...
- 内容：research motivation、research prompts / topic mapping 都不是执行源；prompt 是研究输入，不是研究结果；后续模型应区分 motivation、prompt、report、summary、evidence map、execution source。
- 状态：reflected
- 来源：RAW-0049 / MNEMOSYNE-030F
- 反映位置：raw/chatgpt-discussion-049.md；raw/research-reports/current/research-report-index.md；raw/research-reports/current/current-report-summaries.md

## CAND-0087

- 内容：pro prompt 文件已放入约定路径；6 个轻度研究 prompt 原文缺失，不得编造，也不得把 inferred topic 写成 user original prompt。
- 状态：reflected
- 来源：RAW-0049 / MNEMOSYNE-030F
- 反映位置：current/open-questions.md；current/todo.md；notes/codex-task-results/MNEMOSYNE-030F-result.md
