...
- 内容：需要创建 delivery manifest 模板，用于记录一次目标项目记忆系统交付的总清单、复制范围、人工确认点和验收标准。
- 状态：reflected
- 来源：RAW-0037
- 反映位置：notes/delivery-manifest-template-pack.md

## CAND-0064

- 内容：需要 files to create / update checklist，用于明确目标项目需要创建、更新、复制、合并或跳过的文件。
- 状态：reflected
- 来源：RAW-0037
- 反映位置：notes/delivery-manifest-template-pack.md
...
- 内容：需要 target project runtime truth source checklist，用于确认交付后目标项目执行源、非执行源和运行真相源位置。
- 状态：reflected
- 来源：RAW-0037
- 反映位置：notes/delivery-manifest-template-pack.md
...
- 内容：需要 manual setup steps template，用于记录用户或 AI 工具需要手工完成的目标项目落地步骤。
- 状态：reflected
- 来源：RAW-0037
- 反映位置：notes/delivery-manifest-template-pack.md
...
- 内容：需要 unsupported assumptions linkage，用于把目标项目模板中的未验证假设连接到交付清单和交付阻断判断。
- 状态：reflected
- 来源：RAW-0037
- 反映位置：notes/delivery-manifest-template-pack.md
...
- 内容：需要 delivery review checklist，用于交付前检查目标项目类型、intake、design spec、执行源、文件结构、隐私、unsupported assumptions、rollback、handoff 和 drift review。
- 状态：reflected
- 来源：RAW-0037
- 反映位置：notes/delivery-manifest-template-pack.md
...
- 内容：需要 handoff package template，用于目标项目交付后的跨对话 / 跨任务接手包。
- 状态：reflected
- 来源：RAW-0037
- 反映位置：notes/delivery-manifest-template-pack.md
...
- 内容：需要 rollback / revision plan，用于记录交付失败、覆盖风险或需要回滚时的方案。
- 状态：reflected
- 来源：RAW-0037
- 反映位置：notes/delivery-manifest-template-pack.md
...
- 内容：需要 delivery result record，用于记录一次真实交付后的实际创建、修改、跳过、人工合并、确认和后续任务。
- 状态：reflected
- 来源：RAW-0037
- 反映位置：notes/delivery-manifest-template-pack.md
...
- 内容：delivery manifest template pack 不是执行源，当前执行源仍是 `current/human-approved-spec.md`。
- 状态：reflected
- 来源：RAW-0037
- 反映位置：notes/delivery-manifest-template-pack.md；current/active-context.md；handoff/handoff-current.md

## CAND-0073

- 内容：完成 delivery manifest template pack 后需要用户 review，并根据 review 决定是否小修。
- 状态：reflected
- 来源：RAW-0037
- 反映位置：current/todo.md；current/active-context.md；handoff/handoff-current.md
...
- 内容：需要 delivery manifest template pack 作为 delivery manifest / 目标项目交付包模板的统一入口。
- 状态：reflected
- 来源：RAW-0037；RAW-0038
- 反映位置：notes/delivery-manifest-template-pack.md；current/active-context.md；handoff/handoff-current.md

## CAND-0075

- 内容：需要 minimal delivery runbook，用于描述从读取 intake / design spec 到创建 delivery result record、安排 drift review、提交并 review 的最小交付流程。
- 状态：reflected
- 来源：RAW-0037；RAW-0038
- 反映位置：notes/delivery-manifest-template-pack.md
...
- 内容：需要 delivery completion criteria，用于明确真实目标项目交付至少应满足 manifest、用户确认、执行源、文件清单、manual setup、unsupported assumptions、rollback、handoff、drift review 和阻断问题标注等条件。
- 状态：reflected
- 来源：RAW-0037；RAW-0038
- 反映位置：notes/delivery-manifest-template-pack.md；current/active-context.md

## CAND-0077

- 内容：当前不为真实目标项目生成交付包；delivery manifest template pack 只提供模板和状态同步依据。
- 状态：reflected
- 来源：RAW-0037；RAW-0038
- 反映位置：raw/chatgpt-discussion-038.md；notes/codex-task-results/MNEMOSYNE-028A-result.md

## CAND-0078

- 内容：完成 MNEMOSYNE-028 后需要补齐状态同步和任务结果记录，确保 active-context、handoff、todo、open questions、candidate、decision、roadmap、baseline 与 Codex Task Result Record 一致。
- 状态：reflected
- 来源：RAW-0038
- 反映位置：current/active-context.md；handoff/handoff-current.md；current/todo.md；current/open-questions.md；notes/candidate-requirements.md；notes/decision-log.md；notes/overall-target-and-roadmap-snapshot.md；notes/system-construction-baseline.md；notes/codex-task-results/MNEMOSYNE-028-result.md；notes/codex-task-results/MNEMOSYNE-028A-result.md

## CAND-0079

- 内容：需要对 delivery manifest template pack 状态进行硬同步，确保 active-context、handoff、todo、open-questions、candidate、decision、roadmap、baseline 与 task result record 一致指向“已创建，等待用户 review”。
- 状态：reflected
- 来源：RAW-0039
- 反映位置：current/active-context.md；handoff/handoff-current.md；current/todo.md；current/open-questions.md；notes/candidate-requirements.md；notes/decision-log.md；notes/overall-target-and-roadmap-snapshot.md；notes/system-construction-baseline.md；notes/codex-task-results/MNEMOSYNE-028B-result.md

## CAND-0080

- 内容：delivery manifest template pack 相关候选需求应稳定覆盖 delivery manifest template、files to create / update checklist、target project runtime truth source checklist、manual setup steps template、unsupported assumptions linkage template、delivery review checklist、handoff package template、rollback / revision plan template、delivery result record template、minimal delivery runbook 和 delivery completion criteria。
- 状态：reflected
- 来源：RAW-0037；RAW-0038；RAW-0039
- 反映位置：notes/candidate-requirements.md；notes/delivery-manifest-template-pack.md

## CAND-0081

- 内容：delivery manifest template pack 不是执行源，当前不为真实目标项目生成交付包，完成后仍需要用户 review。
- 状态：reflected
- 来源：RAW-0037；RAW-0038；RAW-0039
- 反映位置：current/active-context.md；handoff/handoff-current.md；current/todo.md；current/open-questions.md；notes/codex-task-results/MNEMOSYNE-028B-result.md

## CAND-0082

- 内容：需要确认 `notes/delivery-manifest-template-pack.md` 已创建，并将其作为 delivery manifest / 目标项目交付包模板的已完成基础能力记录。
- 状态：reflected
- 来源：RAW-0040
- 反映位置：current/active-context.md；handoff/handoff-current.md；notes/overall-target-and-roadmap-snapshot.md；notes/system-construction-baseline.md

## CAND-0083

- 内容：需要将 active-context / handoff-current / todo / open-questions 硬同步到 delivery manifest template pack 已创建、等待用户 review 的状态。
- 状态：reflected
- 来源：RAW-0040
...
- 内容：需要将 delivery manifest 模板深化标记为完成，并保留用户 review 与根据 review 小修 delivery manifest template pack 的后续 TODO。
- 状态：reflected
- 来源：RAW-0040
- 反映位置：current/todo.md；current/active-context.md

## CAND-0085

- 内容：需要将“是否先深化 delivery manifest，还是先选择第一个目标项目场景试用模板？”移入 answered，结论为 MNEMOSYNE-028 已选择先深化 delivery manifest。
- 状态：reflected
- 来源：RAW-0040
- 反映位置：current/open-questions.md
...
- 内容：下一步需要在第一个目标项目场景试用整套模板和进入 Idea Capture Buffer 之间选择。
- 状态：reflected
- 来源：RAW-0040
- 反映位置：current/active-context.md；handoff/handoff-current.md；current/todo.md；current/open-questions.md；notes/overall-target-and-roadmap-snapshot.md；notes/system-construction-baseline.md
