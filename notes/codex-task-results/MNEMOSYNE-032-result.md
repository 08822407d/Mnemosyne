...
## reviewer_correction_after_follow_up

Follow-up verification after MNEMOSYNE-032 found that this result record claimed `handoff/handoff-current.md` had received the Codex / ChatGPT task verification reminder, but direct repository inspection showed that reminder was missing and the handoff read order did not include `notes/codex-task-authoring-and-diff-verification-guidelines.md`.

MNEMOSYNE-032B was created as a hard-fix to:
- insert the missing handoff verification reminder;
- add the guideline file to the handoff read order;
- update `current/active-context.md` from “已准备写入” to “已写入并落账”;
- record this discrepancy as another example of why diff-based verification is required.

## reviewer_correction_after_default_branch_verification

A later default-branch verification found that the original MNEMOSYNE-032 result over-claimed the handoff visibility update: it said `handoff/handoff-current.md` had received the Codex / ChatGPT task verification reminder, but the default branch did not contain that reminder.

MNEMOSYNE-032C was created to fix the final default-branch state by inserting the missing handoff reminder, confirming the guideline read-order item, and updating `current/active-context.md` from “已准备写入” to “已写入并落账”.

This correction is another example of the rule recorded by MNEMOSYNE-032: task result prose is not enough; final repository/default-branch content and git diff evidence must be checked.
