...
- 决策：执行 MNEMOSYNE-025I：当前规范文件中任务结果路径硬清除。
- 理由：当前会被新任务读取的规范文件必须清除旧路径表述，避免后续任务继续写入错误结果文件名。
- 状态：accepted
- 来源：MNEMOSYNE-025I
...
- 决策：Codex Task Result Record 默认占位符路径统一为 `notes/codex-task-results/TASK_ID-result.md`。
- 理由：使用无尖括号的 TASK_ID 占位符降低转义、渲染和模型改写风险。
- 状态：accepted
- 来源：MNEMOSYNE-025I

## DEC-0067

- 决策：不再使用尖括号 TASK_ID 作为默认占位符。
- 理由：尖括号形式容易被 Markdown、HTML、工具或模型处理为特殊标记，增加错误写回风险。
- 状态：accepted
- 来源：MNEMOSYNE-025I
...
- 决策：缺失 TASK_ID 的旧路径不得出现在当前规范文件中。
- 理由：当前规范文件会被后续任务读取，旧路径残留会误导任务结果记录命名。
- 状态：accepted
- 来源：MNEMOSYNE-025I
...
- 决策：Markdown 格式清理可以作为非阻断后续任务，但不应影响路径正确性。
- 理由：路径规范化是进入模板设计前的前置条件；格式清理可独立安排。
- 状态：accepted
- 来源：MNEMOSYNE-025I
