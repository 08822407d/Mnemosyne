...
Meta-Agent 新建专属仓库后的迁移前状态入口：`current/meta-agent-dedicated-repository-pre-migration-status.md`。E0 已由 PR #258 合并：payload snapshot 固定在 `8ef1c43b18b8686a30ffef544ca8b32fce1ca6cb`，根子树为 `4c1cd341777d46b3d6794abc62682e9c915ec46a`，共 226 个 blob；PR #258 的 generator/manifests 属于后置 migration-control evidence，不递归加入 payload。正式裁决：`notes/adjudications/meta-agent-E0-mechanical-inventory-post-merge-and-snapshot-boundary-adjudication-2026-08-06.md`；E1 双平面源合同：`handoff/meta-agent-dedicated-repository-mapping-resume-source-contract-v0.2.yaml`；更新后的 E1 taskbook 与 startup prompt：`handoff/meta-agent-dedicated-repository-mapping-resume-taskbook.md`、`handoff/meta-agent-dedicated-repository-mapping-resume-startup-prompt.md`。这些文件不授权目标仓库初始化、shadow copy、PR 或 cutover。
