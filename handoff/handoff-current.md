...
MNEMOSYNE-031 已完成 R1-R3（用户均选择 B）和 R4A prompt list，并已创建 checkpoint；R4B 由用户延期，R4C 尚未生成，最终 R5 写回包尚未生成。
...
MNEMOSYNE-031 已有 checkpoint record。
...
checkpoint 不是执行源，不是最终 R5 写回包，也不包含用户设计构想重述结果。当前执行源仍是 `current/human-approved-spec.md`。

下一位 assistant 必须从 R4B 继续，不得重启 R1-R3；除非用户明确要求，不得重新生成 R4A。在 R4B / R4C 完成且用户确认前，不得创建 `raw/user-design-restatements/MNEMOSYNE-031-user-design-intent-restatement.md`。
...
27. `raw/research-reports/cycles/2026Q2-initial/review-records/MNEMOSYNE-031-R1-R3-and-R4A-checkpoint.md`
...
- R4B / R4C 完成且用户确认前，不要创建用户设计构想重述记录；
- 不要把 checkpoint 当作最终 MNEMOSYNE-031 R5 写回包；
...
1. 用户准备好后，从 R4B 用户口语化重述作为精确续接点继续：使用已完成的 R4A prompt list，请用户逐项回答；
2. 不要重启或重跑 R1-R3；这些轮次已经完成，并由用户选择 B 接受且保留 review notes；
3. 不要重新生成 R4A，除非用户明确要求修改 prompt list；
4. R4B 完成后生成 R4C 用户设计构想重述结果，并等待用户确认；
5. 在 R4B / R4C 完成并经用户确认前，不创建 `raw/user-design-restatements/MNEMOSYNE-031-user-design-intent-restatement.md`；
6. R4C 确认后生成最终 R5 combined writeback package。
