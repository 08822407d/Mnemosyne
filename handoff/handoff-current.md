...
16. `raw/research-reports/current/research-report-index.md`
17. `raw/research-reports/current/current-evidence-map.md`
18. `raw/research-reports/current/current-capability-boundaries.md`
19. `raw/concept-origin-extract-001.md` 按需回查
...
- 不要把 candidate / decision / active-context / handoff / startup-instructions / self-improvement template pack / target project template pack / delivery manifest template pack 当执行源。

## 下一步建议

1. 用户 review delivery manifest template pack；
2. 根据 review 小修模板；
3. 决定选择第一个目标项目场景，还是进入 Idea Capture Buffer。
